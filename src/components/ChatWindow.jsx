...
const ChatWindow = () => {
  ...
  // Función para volver atrás (útil si luego agregamos versión móvil)
  const closeChat = () => {
    dispatch({ type: "CHANGE_USER", payload: null }); // O resetear según tu reducer
  };
    ...
    >
      ...
      <div className="h-20 bg-white/80 backdrop-blur-md border-b border-gray-200/50 flex items-center justify-between px-6 shadow-sm z-20 sticky top-0">
        <div className="flex items-center gap-4">
            {/* Botón atrás (visible solo si quisieras hacerlo responsive luego) */}
      ...
      {/* 2. ÁREA DE MENSAJES */}
      {/* Usamos 'pb-24' (padding bottom) para dejar espacio al Input flotante */}
      ...
      {/* 3. INPUT (Caja de escritura) */}
      {/* Se renderiza aquí, pero en Input.jsx le pusimos 'absolute bottom-0' */}
      <Input />
    </div>
  );
};
